...
namespace TownOfHost
{
    ...
    public class CustomHatLoader
    {
        ...
        public static string[] hatReports = new string[]
        {
        ...
        private static async Task LaunchHatFetcherAsync()
        {
            ...
            List<string> reports = new List<string>(hatReports);

            if (main.DebugReport.Value != "")
                reports.Insert(0, main.DebugReport.Value);

            foreach (string repo in reports)
            {
                try
                {
